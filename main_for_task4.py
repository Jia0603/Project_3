...
import common.boundary_config as bc
...
def parse_arguments():
    ...
    parser.add_argument('--omega', type=float, default=0.8,
                        help='Relaxation parameter ω (default: 0.8)')
    parser.add_argument('--iters', type=int, default=10,
                        help='Number of D–N iterations K (default: 10)')
    parser.add_argument('--dx', type=float, default=None,
                        help='Grid spacing Δx (default: use get_default_dx())')
    parser.add_argument('--dy', type=float, default=None,
                        help='Grid spacing Δy (default: use get_default_dy())')
    parser.add_argument('--heater-temp', type=float, default=None,
                        help='Override heater temperature (affects boundary_config.HEATER_TEMP)')
...
def main(apt_new=False, dx=None, dy=None, omega=0.8, num_iters=10, heater_temp=None):
    # 解析/回退默认网格参数
    dx = dx if dx is not None else get_default_dx()
    dy = dy if dy is not None else get_default_dy()
    # 覆盖暖气片温度（修改 boundary_config 模块中的常量）
    if heater_temp is not None:
        bc.HEATER_TEMP = heater_temp
...
"""
    ...
    plt.ylabel('y [m]')
    plt.colorbar();
    plt.title("Room1 Temperature (with boundaries)")
    room1_path = os.path.join(out_dir, "room1.png")
    plt.savefig(room1_path, dpi=200);
    plt.close()
    ...
    plt.ylabel('y [m]')
    plt.colorbar();
    plt.title("Room2 Temperature (with boundaries)")
    room2_path = os.path.join(out_dir, "room2.png")
    plt.savefig(room2_path, dpi=200);
    plt.close()
    ...
    plt.ylabel('y [m]')
    plt.colorbar();
    plt.title("Room3 Temperature (with boundaries)")
    room3_path = os.path.join(out_dir, "room3.png")
    plt.savefig(room3_path, dpi=200);
    plt.close()
    ...
    if apt_new == True:
        ...
        plt.ylabel('y [m]')
        plt.colorbar();
        plt.title("Room4 Temperature (with boundaries)")
        room4_path = os.path.join(out_dir, "room4.png")
        plt.savefig(room4_path, dpi=200);
        plt.close()
