...
def _solve_room(room_id, gamma1, gamma2, dx, dy, solver_config=None):
    ...
    """
    if solver_config is None: # if the solver configuration is not provided, use the default configuration
        solver_config = {'solver_type': 'spsolve', 'tol': 1e-6, 'maxiter': 10000} # default solver configuration

    info = get_room_grid_info(room_id, dx, dy) # get the information of the room
    
    # Generates a description of the mixed boundary conditions of the current room: 
    # whether each boundary is Dirichlet or Neumann, and the corresponding values
    bc_types, bc_values = get_boundary_conditions(room_id, gamma1, gamma2, None,  dx, dy) # get the boundary conditions of the current room

    A, nx_solve, ny_solve = build_laplace_matrix_mixed(info["Nx"], info["Ny"], dx, bc_types) # build the Laplace matrix of the current room
    b = build_b_mixed(info["Nx"], info["Ny"], dx, bc_types, bc_values) # build the right-hand side of the current room
    ...
    # Select solver
    solver_type = solver_config.get('solver_type', 'spsolve') # get the solver type from the solver configuration
    ...
    if solver_type == 'cg':
        ...
        if hasattr(A, 'toarray'): # if the matrix is sparse, convert it to a dense matrix
            # Convert sparse to dense then solve
            u_flat = np.linalg.solve(A.toarray(), b) # solve the system of equations
        else: # if the matrix is dense, solve it directly
            # Dense matrix
            u_flat = np.linalg.solve(A, b) # solve the system of equations

    # In matrix_builder.py, the two-dimensional grid points are linearized into a one-dimensional vector, 
    # and the storage order is: first store by row, then store by column
    u = u_flat.reshape(ny_solve, nx_solve) # reshape the solution to the original shape
    return u
...
def dirichlet_neumann_iterate(dx, dy, omega=0.8, num_iters=10, solver_config=None):
    ...
    # getting the communication domain, current rank and total number of processes.
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()

    # Validate number of processes (must be 4: root + 3 rooms)
    if size != 4:
        if rank == 0:
            raise ValueError("This simulation requires 4 MPI ranks. Run with 'mpiexec -n 4 python main.py'")
        comm.Abort()

    # Initialize interface arrays (length follows room1 Ny) - get the number of grids along y axis in room 1
    room1_info = get_room_grid_info("room1", dx, dy) # get the information of room 1
    Ny_interface = room1_info["Ny"] # get the number of grids along y axis in room 1
    gamma1 = np.full(Ny_interface, 20.0, dtype=float) # initialize the interface temperatures of room 1 and room 2
    gamma2 = np.full(Ny_interface, 20.0, dtype=float) # initialize the interface temperatures of room 2 and room 3

    # MPI tags for communication between processes
    ...
    if rank == 0:
        ...
        for _ in range(num_iters):
            ...
            comm.bcast(gamma1, root=0) # broadcast the interface temperatures of room 1 and room 2 to the workers
            comm.bcast(gamma2, root=0) # broadcast the interface temperatures of room 2 and room 3 to the workers

            # Receive solutions from workers
            u1 = comm.recv(source=1, tag=TAG_G1_SEND) # receive the solution from room 1
            _ = comm.recv(source=2, tag=TAG_G1_SEND)  # Room2 solution not used in interface update
            u3 = comm.recv(source=3, tag=TAG_G1_SEND) # receive the solution from room 3
            ...
            gamma1_new[1:-1] = omega * u1_right + (1 - omega) * gamma1[1:-1] # update the interface temperatures of room 1 and room 2
            gamma2_new[1:-1] = omega * u3_left + (1 - omega) * gamma2[1:-1] # update the interface temperatures of room 2 and room 3
            
            gamma1, gamma2 = gamma1_new, gamma2_new # update the interface temperatures of room 1 and room 2

        # Notify workers that iterations are complete - send a termination signal to the workers
        for dst_rank in (1, 2, 3):
            comm.send(True, dest=dst_rank, tag=TAG_DONE)

        # Broadcast final interfaces and gather final solutions
        comm.bcast(gamma1, root=0) # broadcast the final interface temperatures of room 1 and room 2 to the workers
        comm.bcast(gamma2, root=0) # broadcast the final interface temperatures of room 2 and room 3 to the workers
        u1 = comm.recv(source=1, tag=TAG_G2_SEND) # receive the solution from room 1
        u2 = comm.recv(source=2, tag=TAG_G2_SEND) # receive the solution from room 2
        u3 = comm.recv(source=3, tag=TAG_G2_SEND) # receive the solution from room 3

        return {"u1": u1, "u2": u2, "u3": u3, "gamma1": gamma1, "gamma2": gamma2} # return the final solutions and interface temperatures

    # Worker logic (rank1: room1, rank2: room2, rank3: room3)
    if rank in (1, 2, 3):
        # Map rank to room id - map the current rank to the corresponding room
        room_id_map = {1: "room1", 2: "room2", 3: "room3"}  
        room_id = room_id_map[rank] # get the room id of the current rank

        # Iteration phase: receive interfaces, solve subdomain, send to root
        for _ in range(num_iters):
            # Receive interface temperatures
            gamma1 = comm.bcast(None, root=0) # broadcast the interface temperatures of room 1 and room 2 to the current rank
            gamma2 = comm.bcast(None, root=0) # broadcast the interface temperatures of room 2 and room 3 to the current rank
        ...
        _ = comm.recv(source=0, tag=TAG_DONE) # receive a termination signal from the root

        # Final phase: receive final interfaces, solve, and send final solution
        gamma1 = comm.bcast(None, root=0) # broadcast the final interface temperatures of room 1 and room 2 to the current rank
        gamma2 = comm.bcast(None, root=0) # broadcast the final interface temperatures of room 2 and room 3 to the current rank
        final_u = _solve_room(room_id, gamma1, gamma2, dx, dy, solver_config)  # solve the final subdomain
        comm.send(final_u, dest=0, tag=TAG_G2_SEND) # send the final solution to the root
